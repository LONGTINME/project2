...
RSU::RSU(const ElGamal::PublicKey &rsuPublicKey, const ElGamal::PrivateKey &rsuPrivateKey, const ElGamal::PublicKey &esPublicKey)
    : rsuPublicKey_(rsuPublicKey), rsuPrivateKey_(rsuPrivateKey), esPublicKey_(esPublicKey), currentIdx_(0) {
...
void RSU::PrepareDataForES(std::string &encryptedPseudonymAndTime, std::string &encryptedTask) {
    std::string pseudonymAndTime = currentFN_ + "||" + currentTime_;
    encryptedPseudonymAndTime = ElgamalEncrypt(pseudonymAndTime, esPublicKey_);
    encryptedTask = currentTask_;
}
void RSU::SendTaskToES(ES &es) {
    std::string encryptedPseudonymAndTime;
    std::string encryptedTask;
    PrepareDataForES(encryptedPseudonymAndTime, encryptedTask);
    es.ReceiveTask(encryptedPseudonymAndTime, encryptedTask);
}
