...
void Vehicle::PrepareDataForRSU(std::string &encryptedIdentity, std::string &encryptedTaskAndNoise) const {
    std::cout << "Encrypting identity and task..." << std::endl;

    std::string identity = std::to_string(id_);
    encryptedIdentity = ElgamalEncrypt(identity, rsuPublicKey_);

    std::string taskAndNoise = currentTask_ + "||" + randomNoise_;
    encryptedTaskAndNoise = ElgamalEncrypt(taskAndNoise, esPublicKey_);
}
void Vehicle::SendTaskToRSU(RSU &rsu) {
    ...
    PrepareDataForRSU(encryptedIdentity, encryptedTaskAndNoise);
